...
def main():
    ...
    draw(
        ...
        gap_factor=1.42, lbl_shift=0.06,   # 위쪽 gap 확장 + 레이블 약간 아래로
    )

    cx2, cy2 = 1.6 * s5, 0.8 * s5   # A=(cx2,cy2) ≈ (3.578, 1.789): 기하학적 유일해
    # ※ B=(0,0), C=(2√5,0) 고정 시 AB=4, AC=2, 직각at A 조건의 유일 해.
    #   A를 "조금만" 이동하는 것은 불가(BC 수평 유지 전제). 더 이동하려면 회전 필요.
    ...
    draw(
        ...
        vertex_label_rotations={'A': 40},   # A 레이블을 CCW 40° 회전 → 위쪽으로 이동
    )
    ...
    draw(
        ...
        vertex_label_rotations={'A': 12},   # A 레이블 CCW 12° → 우측으로 미세 이동
    )
    ...
    draw(
        ...
        vertex_label_rotations={'A': 12},   # A 레이블 CCW 12° → 좌측으로 미세 이동
    )

    # ── 005: 002 좌우대칭 ────────────────────────────────────────────────────
    # A=(-cx2,cy2), B=(-2√5,0), C=(0,0) — BC 수평 유지
    draw(
        {'A': (-cx2, cy2), 'B': (-2 * s5, 0), 'C': (0, 0)},
        right_v='A',
        side_labels={'AB': r'$2$', 'AC': r'$4$'},
        filename='sample_005.png',
        vertex_label_rotations={'A': -40},  # 002의 좌우대칭: CW 40°
    )
    ...
    # ── edited01 세트: 기존 샘플과 비교용 (_edited01 접미사) ──────────────────
    print("edited01 세트 생성 중...")
    ...
    draw(
        ...
        right_v='B',
        side_labels={'AB': r'$4$', 'BC': r'$2$', 'AC': r'$2\sqrt{5}$'},
        filename='sample_001_edited01.png',
        gap_factor=1.42, lbl_shift=0.06,
        vertex_label_rotations={'A': 10},
    )

    # 002_edited01: A 40° +5° = 45°
    draw(
        {'A': (cx2, cy2), 'B': (0, 0), 'C': (2 * s5, 0)},
        right_v='A',
        side_labels={'AB': r'$4$', 'AC': r'$2$'},
        filename='sample_002_edited01.png',
        vertex_label_rotations={'A': 45},
    )

    # 003_edited01: B 아래로(CW=-10°), 숫자3 살짝 아래로(CB shift 7%)
    draw(
        {'A': (0, 0), 'B': (3, 6), 'C': (0, 6)},
        right_v='C',
        side_labels={'CA': r'$6$', 'CB': r'$3$'},
        filename='sample_003_edited01.png',
        vertex_label_rotations={'A': 12, 'B': -10},
        side_label_shifts={'BC': 0.07},
    )

    # 005_edited01: A -40° -5° = -45°
    draw(
        {'A': (-cx2, cy2), 'B': (-2 * s5, 0), 'C': (0, 0)},
        right_v='A',
        side_labels={'AB': r'$2$', 'AC': r'$4$'},
        filename='sample_005_edited01.png',
        vertex_label_rotations={'A': -45},
    )

    # 006_edited01: A -10°, C 아래로(CCW=+10°)
    draw(
        {'A': (0, 0), 'B': (0, 6), 'C': (-3, 6)},
        right_v='B',
        side_labels={'AB': r'$6$', 'BC': r'$3$'},
        filename='sample_006_edited01.png',
        vertex_label_rotations={'A': -10, 'C': 10},
    )

    print("[완료] edited01 세트 저장됨")

    # ── edited02 세트 ─────────────────────────────────────────────────────────
    print("edited02 세트 생성 중...")
    ...
    draw(
        ...
        right_v='B',
        side_labels={'AB': r'$4$', 'BC': r'$2$', 'AC': r'$2\sqrt{5}$'},
        filename='sample_001_edited02.png',
        gap_factor=1.42, lbl_shift=0.06,
        vertex_label_rotations={'A': -10},
    )
    ...
    draw(
        ...
        right_v='A',
        side_labels={'AB': r'$4$', 'AC': r'$2$'},
        filename='sample_002_edited02.png',
        vertex_label_rotations={'A': 30},
    )

    # 003_edited02: 숫자 3 더 아래로 (CB shift 15%)
    draw(
        {'A': (0, 0), 'B': (3, 6), 'C': (0, 6)},
        right_v='C',
        side_labels={'CA': r'$6$', 'CB': r'$3$'},
        filename='sample_003_edited02.png',
        vertex_label_rotations={'A': 12},
        side_label_shifts={'BC': 0.25},
    )
    ...
    draw(
        ...
        right_v='A',
        side_labels={'AB': r'$2$', 'AC': r'$4$'},
        filename='sample_005_edited02.png',
        vertex_label_rotations={'A': -35},
    )

    print("[완료] edited02 세트 저장됨")

    # ── edited03: 003 숫자3 위치 CB shift 0.25 ────────────────────────────────
    print("edited03 세트 생성 중...")

    draw(
        {'A': (0, 0), 'B': (3, 6), 'C': (0, 6)},
        right_v='C',
        side_labels={'CA': r'$6$', 'CB': r'$3$'},
        filename='sample_003_edited03.png',
        vertex_label_rotations={'A': 12},
        side_label_shifts={'BC': 0.25},
    )

    print("[완료] edited03 저장됨")


    # ── edited04: 003 숫자3 y 직접 오프셋 -0.25 ─────────────────────────────
    print("edited04 생성 중...")
    ...
    draw(
        ...
        right_v='C',
        side_labels={'CA': r'$6$', 'CB': r'$3$'},
        filename='sample_003_edited04.png',
        vertex_label_rotations={'A': 12},
        side_label_offsets={'BC': (0, -0.25)},
    )

    print("[완료] edited04 저장됨")

    # ── edited05: 003 숫자3 offset (0,-0.45) → CB 변 위에 표시 ─────────────
    print("edited05 생성 중...")
    ...
    draw(
        ...
        right_v='C',
        side_labels={'CA': r'$6$', 'CB': r'$3$'},
        filename='sample_003_edited05.png',
        vertex_label_rotations={'A': 12},
        side_label_offsets={'BC': (0, -0.65)},
    )

    print("[완료] edited05 저장됨")

    # ── edited06: 003 숫자3 offset (0,-0.15) → 너무 내려간 것 적절히 수정 ──────────
    print("edited06 생성 중...")
    ...
    draw(
        ...
        right_v='C',
        side_labels={'CA': r'$6$', 'CB': r'$3$'},
        filename='sample_003_edited06.png',
        vertex_label_rotations={'A': 12},
        side_label_offsets={'BC': (0, -0.15)},
    )

    print("[완료] edited06 저장됨")
