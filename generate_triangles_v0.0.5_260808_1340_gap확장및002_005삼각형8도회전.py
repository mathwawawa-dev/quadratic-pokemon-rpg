...
def draw(vertices_dict, right_v, side_labels, filename):
    ...
    for (v1, v2, label, ap, peak) in arcs:
        ...
        excl_r = max(half_w, half_h) * 1.42 + dash_cycle_data * 0.55  # 1.35→1.42: 윗부분 gap 미세 확장
...
def main():
    ...
    # 002/005: 삼각형을 8° CCW 회전 → A가 짧은 변 끝(C or B) 바로 위에 몰리는 현상 완화
    # (회전 전: A=(3.578,1.789), C=(4.472,0) → A와 C가 x좌표 가까움 → 불안정)
    rot_rad = math.radians(8)
    cr, sr  = math.cos(rot_rad), math.sin(rot_rad)
    cx2_base, cy2_base = 1.6 * s5, 0.8 * s5   # 회전 전 기본 좌표
    # CCW 8° 회전: (x,y) → (x·cosθ − y·sinθ, x·sinθ + y·cosθ)
    cx2  = cx2_base * cr - cy2_base * sr   # ≈ 3.294 (이전 3.578, Δ=−0.284)
    cy2  = cx2_base * sr + cy2_base * cr   # ≈ 2.269 (이전 1.789, Δ=+0.480)
    c002x = 2 * s5 * cr                    # ≈ 4.428 (이전 4.472)
    c002y = 2 * s5 * sr                    # ≈ 0.622 (이전 0.000)

    # ── 002 ──────────────────────────────────────────────────────────────────
    draw(
        {'A': (cx2, cy2), 'B': (0, 0), 'C': (c002x, c002y)},
        right_v='A',
    ...
    # ── 005: 002 좌우대칭 ────────────────────────────────────────────────────
    # 002 회전 좌표를 x→−x 로 미러 → A가 오른쪽으로 이동 (−3.578→−3.294, Δ=+0.284)
    draw(
        {'A': (-cx2, cy2), 'B': (-c002x, c002y), 'C': (0, 0)},
        right_v='A',
