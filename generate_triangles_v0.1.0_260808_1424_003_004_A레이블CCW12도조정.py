...
def draw(vertices_dict, right_v, side_labels, filename,
         gap_factor=1.35, lbl_shift=0.0, vertex_label_rotations=None):
    """
    gap_factor             : excl_r 여백 계수 (v0.0.4 기본=1.35)
    lbl_shift              : 레이블 중심을 v2 방향으로 이동하는 호 인덱스 비율 (0.0=peak 정중앙)
    vertex_label_rotations : dict {'A': 40} 등. 무게중심->꼭짓점 방향을 CCW 도만큼 회전해 레이블 위치 조정
    """
    ...
    for (v1, v2, label, ap, peak) in arcs:
        ...
        # ② 레이블 중심: peak에서 v2 방향으로 lbl_shift 비율만큼 이동
        #    (lbl_shift=0.0 → peak 정중앙, v0.0.4 동일)
        peak_idx = int(np.argmin(np.linalg.norm(ap - peak, axis=1)))
        if lbl_shift > 0.0:
            shift_n  = max(1, int(n * lbl_shift))
