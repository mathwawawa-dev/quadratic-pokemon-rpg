...
def draw(vertices_dict, right_v, side_labels, filename,
    ...
    for (v1, v2, label, ap, peak) in arcs:
        ...
        dists  = np.linalg.norm(ap - lbl_peak, axis=1)
        ...
        # ④ 레이블: 이동된 lbl_peak 위치 + side_label_offsets 직접 오프셋
        _slo = side_label_offsets or {}
        off  = _slo.get(v1 + v2, (0.0, 0.0))
        tx, ty = lbl_peak[0] + off[0], lbl_peak[1] + off[1]
        ax.text(tx, ty, label,
