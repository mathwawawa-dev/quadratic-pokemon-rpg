...
// 1. Remove the old buggy cave render block
const buggyRegex = /\/\/ Cave ceiling\/wall overlay \([^\)]+\)[\s\S]*?(?=if \(tData\.islands\) \{)/;
code = code.replace(buggyRegex, '');

// 2. Insert the correct cave render block right before `// Terrain polygon`
const correctCaveRender = `
...
code = code.replace(
    '    // Terrain polygon',
    correctCaveRender + '    // Terrain polygon'
);

fs.writeFileSync('js/engine.js', code);
console.log("Cave render repositioned and fixed successfully.");
